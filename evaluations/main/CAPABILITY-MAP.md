...
| C7 build/run | r01 `unica.run`; **r22 CLI `build`/`launch` + `--json-message`** | частичное→сильное у r22 | c22-entry; c22-json-envelope; c22-ib-simple; c01-entry; c22-e2b-two-ibs-simple | r22 init/build RUN; r01 static; **две ИБ одновременно (два `--config`/два процесса) RUN, изоляция подтверждена** (E2b) | launch/apply |
| C8 runtime/UI test | **r22 YaXUnit CLI + `test va`**; **r20 Answer42**; **r21 Vanessa (движок)** | частичное, YaXUnit — открытый риск | c22-entry; c22-va-smoke-simple; c20-entry; c20-mcp-simple; c20-dual-live; c21-cli-target; c22-yaxunit-crossext-discovery-unresolved-simple; c22-yaxunit-crossext-hypotheses-ruled-out-simple | Vanessa engine smoke RUN 1/1 (через r22 `test va`); Answer42 smoke RUN; dual live r20 RUN; движок YaXUnit и тестовое расширение установлены/активны (build/load/merge/SafeMode — PASS), но `test yaxunit` находит 0 сценариев: модуль стороннего расширения не резолвится ни статически, ни динамически изнутри YAXUNIT (`Метод объекта не обнаружен`) | причина кросс-расширенческого резолвинга YaXUnit (Designer/EDT-расширение для сравнения недоступно в среде; несколько гипотез исключены); click_button r20 |
...
Все связи между слоями фасада `PROPOSED_INTEGRATION` — сам фасад
(код-обёртка) не писался и не запускался, `VERIFIED_INTEGRATION` для
него нет. Отдельные слои за пять продуктов при этом прогнаны живьём
существенно шире dump-only: r15 CLI+MCP+whitelist (HTTP и stdio), r22
init/build/syntax/dump/две ИБ одновременно, r01
`view`/`check`/`apply` dryRun и публикации/`docs`/два source-set в
одном yaml, **r20 Answer42 smoke и dual live Test Client**, r21 Vanessa
через `r22 test va`. Сквозной проход по всей цепочке (explore → … →
verify.scenario) выполнен один раз — [logs/e3-chain-simple.md](logs/e3-chain-simple.md);
единственное исключение — `verify.unit` (YaXUnit), где сам прогон
случился, но результат FAIL по кросс-расширенческой находке, не PASS.
r01/r15/r20/r21/r22 прочитаны в объёме S1–S5.

## Пробелы

1. **`verify.unit` через YaXUnit — открытый риск, не закрыт.** Движок и
   тестовое расширение реально установлены и активны на стенде `simple`
   (build/load/merge/SafeMode — всё PASS), но обнаружение тестового
   модуля соседним расширением не работает ни статически, ни динамически
   (`Метод объекта не обнаружен`). Проверены и исключены: порядок/
   приоритет расширений, алфавитное имя, `filter.extensions` в
   config.json YaXUnit, момент пересборки после переименования. Не
   проверено: тестовое расширение, собранное настоящим Дизайнером/EDT
   (недоступны в среде), обращение в официальную поддержку 1С. Подробно:
