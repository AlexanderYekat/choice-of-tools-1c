...
| S3 | Узкая surface | SOURCE: ровно 11 tools; фасад зовёт subset docs/view/apply/check | SOURCE+DOCUMENTATION: ~20+13; CLI без tools/list; `[tools].enabled` режет MCP | SOURCE: 122 `@mcp.tool()` при `full`; `--tool-profile` / `--disable-rag` режут список | SOURCE+INFERENCE: CLI без MCP tools; MCP 37 statically active, скрывать фасадом | SOURCE: CLI без tools/list; MCP ровно 8 `#[tool]` |
...
Static пяти закрыт. Следующий шаг — ограниченный execution по отдельному
разрешению, не код фасада.
