...
R01_E2A_MULTI_SOURCESET_PASS; R22_E2B_TWO_IBS_PASS.
...
3. **E3 сквозной прогон и цена контекста — ТЕКУЩИЙ ШАГ.** Один проход explore → docs
   → edit → static → build → verify.unit → verify.form → verify.scenario
   последовательностью вызовов, с замером объёма выдач по каждой
   операции поверхности. Здесь же снять вопрос smoke-`Comment` в
   `.v8/work/simple-cf-220`: повторный Designer dump его затрёт.
   `verify.unit` в этом проходе будет BLOCKED/FAIL из-за E1, если не
   найдётся способ.
4. **E4 r15 stdio-whitelist и `--path` + `--config`** — догоняющее,
   ничего не блокирует.
