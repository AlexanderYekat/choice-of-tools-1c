...
7. [FACADE-TZ.md](FACADE-TZ.md) — синтез S1–S5 и E1–E4 в методологическое
   ТЗ для будущего кода фасада; читать перед написанием фасада.
8. [FACADE-TECH-SPEC.md](FACADE-TECH-SPEC.md) — техническое ТЗ поверх
   методологического: компоненты, конфиги, envelope, таксономия ошибок,
   алгоритмы по стыкам; читать вместе с FACADE-TZ.md перед написанием
   кода.
9. [FACADE-IMPLEMENTATION-PLAN.md](FACADE-IMPLEMENTATION-PLAN.md) —
   техническое ТЗ, разбитое на 11 чекпоинтов (CP0–CP10) с критериями
   приёмки; читать, если разрешение писать код уже дано и нужно
   выбрать, с чего начинать.
10. Для r01 — [reports/r01.md](reports/r01.md); для r15 —
...
[logs/r15-e4-stdio-whitelist-simple.md](logs/r15-e4-stdio-whitelist-simple.md).
2026-09-22 → «зафиксируй этап»: пользователь принял решение закрыть
этап исполнения с открытым риском E1, не продолжать изоляцию причины в
рамках текущей очереди. Обновлены [REPORT.md](REPORT.md) и
[CAPABILITY-MAP.md](CAPABILITY-MAP.md): C7 (build/run) — две ИБ
одновременно теперь RUN, а не пробел; C8 (runtime/UI test) — YaXUnit
помечен как открытый риск с точной причиной, а не устаревшее «нет
расширения»; «Пробелы» переписаны под актуальную находку E1.
[INTEGRATION-PLAN.md](INTEGRATION-PLAN.md) — статус наверху и «Критерии
завершения этапа» отмечены построчно (✅/❌), с явной записью решения
пользователя. Ни один факт не переоценён в сторону PASS — `verify.unit`
остаётся FAIL. Фасад не писался.
2026-09-22 → «сделай анализ всех предыдущих этапов и создай
методологическое ТЗ для фасада»: составлен
[FACADE-TZ.md](FACADE-TZ.md) — синтез static S1–S5 и execution E1–E4
в единый методологический документ: состав контура, внешняя
поверхность, контракт адресации цели, контракт по каждой из 7 операций
(explore/docs/edit.view/edit.apply/static/verify.unit/verify.form/
verify.scenario) с реальными размерами ответов из E3, определение
1С-проекта, 7 обязательных стыков между продуктами из E3, открытый
риск E1 и требование к фасаду его честно отражать, 7 нефункциональных
принципов из FACADE.md, критерии приёмки по C1–C14 с явной пометкой
`PROPOSED_INTEGRATION` (не `VERIFIED_INTEGRATION` — код фасада не
существует), список из 6 открытых вопросов до начала кода. Синтез
составлен из уже существующих файлов запуска, новых прогонов не
выполнялось, факты и статусы не менялись. Запрет писать код фасада не
снят этим документом.
2026-09-22 → «ответь по умолчанию, не усложняя»: 6 открытых вопросов
[FACADE-TZ.md](FACADE-TZ.md) закрыты рабочими дефолтами, не новыми
проверенными фактами: (1) маркеры автоопределения — закрытый список из
трёх, `Configuration.xml` → EDT `Configuration.mdo` → `v8facade.yaml`;
(2) манифест — `.v8/facade.yaml`, 4 поля на старте; (3) credentials —
`.env`-подобный `.v8/credentials.local`, в `.gitignore`; (4) source vs
target-дамп — единая запись, разведение только по факту отказа
`invalid_source`/`source_unreadable`; (5) E1 — не продолжать изоляцию
сейчас, `verify.unit` в первой версии фасада честно `ok=false`; (6)
диагностический долг стенда — откатить перед следующим использованием
стенда, не перед написанием кода. Правки только в FACADE-TZ.md; прогонов
не было.
2026-09-22 → «составь техническое ТЗ, на основе методологического»:
создан [FACADE-TECH-SPEC.md](FACADE-TECH-SPEC.md) — технический слой
поверх [FACADE-TZ.md](FACADE-TZ.md): компоненты (Router/ProjectDetector/
ConfigLoader/TargetRegistry/5 адаптеров/ProcessManager/ArtifactStore/
EnvelopeNormalizer), схема `.v8/facade.yaml` и `.v8/credentials.local`,
единый envelope ответа, таксономия ошибок фасада с привязкой к
наблюдённым продуктовым кодам (`provider_unavailable`,
`source_format_unwritable`, `stale_revision`, `extension_name_mismatch`,
`verify_unavailable` и др.), порог инлайна 8192 байт и конвенция
`ArtifactStore`, технические алгоритмы по всем 7 стыкам из FACADE-TZ.md,
честная реализация открытого риска E1 (`verify.unit` обязан отвечать
`ok=false` с `verify_unavailable`, не подменяться другим слоем),
жизненный цикл процессов по всем пяти продуктам, приёмочный чек-лист на
стенде `simple` как воспроизведение уже существующих логов через фасад.
Явно помечено нерешённым: язык/рантайм реализации (дефолт — Python
3.11+, stdio MCP сервер) и место размещения кода (вне `evaluations/main`)
— предложены как дефолты документа, не подтверждены пользователем.
Запрет писать код фасада не снят. Новых прогонов не выполнялось.
2026-09-22 → «исследуй репозиторий https://github.com/AlexanderYekat/
v8-harness. возможно стоит сделать на его основе»: прочитан исходник
(`v8_harness.py`, 935 строк, только stdlib, 19 unit-тестов), README,
AGENTS.md и skill в локальном клоне
`C:\Users\Enduro\Documents\1c\Tools\v8-harness`
(`origin/master`=`105b3aa4f162187e223d6aab4e3a6c0794fd0476`, remote
совпадает с указанным URL). Это не один из 22 кандидатов — отдельный
инструмент того же пользователя, уже подключённый в этом запуске как
opt-in fixture (маркер `v8stands.yaml`, стенд `simple`). Решение: взять
за основу для слоя ConfigLoader/TargetRegistry в части r22/Vanessa —
он уже решает «несколько именованных целей в одном проекте», адресацию
ИБ по `launcher`/`file`/`connection`/`cf`, разделение секретов
(`v8project.local.yaml`, не в git) и условную загрузку Vanessa/YaXUnit;
не решает и не заменяет остальную архитектуру фасада (Router, 4 из 5
адаптеров, envelope, ArtifactStore, бюджет контекста) — они остаются
как есть. [FACADE-TZ.md](FACADE-TZ.md): добавлен раздел 2a; раздел 6
(определение проекта) пересмотрен на строгий opt-in по единственному
маркеру `v8stands.yaml` вместо автоопределения по структуре выгрузки
(это уже действующая политика в `AGENTS.md` этого репозитория, не
новая гипотеза); раздел 4 и пп. 1–4 раздела 14 переписаны под реальную
схему v8-harness вместо придуманных `.v8/facade.yaml`/
`.v8/credentials.local`. [FACADE-TECH-SPEC.md](FACADE-TECH-SPEC.md):
компоненты, схема конфига §3, таксономия ошибок §5, стыки §7.1/7.4/7.5,
таблица процессов §9 (строка r22) и приёмочный чек-лист §10 приведены
в соответствие — `RunnerAdapter` вызывает `v8-harness run --stand <id>
-- …` (или импортирует как библиотеку), не собирает `--config` сам.
Формальная регистрация v8-harness как кандидата в
`INVENTORY.json`/`EVIDENCE.json`/`DECISION.md`/`CAPABILITY-MAP.md` не
выполнялась — вне запроса пользователя на этот шаг. Запрет писать код
фасада не снят. Прогонов не выполнялось.
2026-09-22 → «разбей техническое ТЗ на план реализации с чекпоинтами.
Оформи это файлом»: создан
[FACADE-IMPLEMENTATION-PLAN.md](FACADE-IMPLEMENTATION-PLAN.md) — 11
чекпоинтов (CP0–CP10) по возрастанию риска и зависимостей: CP0
разрешение писать код + подтверждение трёх нерешённых дефолтов стека;
CP1 ConfigLoader/TargetRegistry/ProjectDetector поверх v8-harness; CP2
envelope/таксономия ошибок/ArtifactStore/пустой Router; CP3 IndexAdapter
(`explore`, выбран первым как самый простой); CP4 UnicaAdapter (`docs`
асинхронность, `edit.apply` dryRun/`ifRev`, разведение формата 2.20/1.0);
CP5 RunnerAdapter через `v8-harness run --stand` (build fallback, имя
расширения, честная обработка открытого риска E1 — обязательный
регрессионный тест на `ok=false`); CP6 FormAdapter (Answer42); CP7
ScenarioAdapter (Vanessa через CP5); CP8 два target одновременно; CP9
бюджет контекста на полном цикле (≈30 КБ); CP10 приёмка по всем 9
пунктам §10 техТЗ и решение пользователя о продолжении. Каждый
чекпоинт — с критерием приёмки, сверяемым с уже существующими логами
static-стыковки, не новыми придуманными тестами. Запрет писать код
фасада не снят — план это явно фиксирует как гейт CP0. Прогонов не
выполнялось.
